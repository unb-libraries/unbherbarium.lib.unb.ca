...
class HerbariumImageSurrogateFactory {
  ...
  protected function generateJpgSurrogate(&$context) {
    ...
    \Drupal\Core\File\FileSystemInterface::prepareDirectory($target_path_u, \Drupal\Core\File\FileSystemInterface::CREATE_DIRECTORY);
    $file_destination_u = "$target_path_u/$nid-$uniqid.jpg";
    $uri_u = \Drupal::service('file_system')->copy($temp_image_file, $file_destination_u, \Drupal\Core\File\FileSystemInterface::EXISTS_REPLACE);
    $file_u = File::Create([
      'uri' => $uri_u,
    ]);
    $file_u->setPermanent();
